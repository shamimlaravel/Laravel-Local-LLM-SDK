...
class LMStudioDriver extends BaseDriver
{
    public function getDriver(): Driver
    {
        return Driver::LM_STUDIO;
    }
    ...
    public function models(): array
    {
        try {
            ...
            return array_map(
                static fn(array $model) => ModelInfo::fromArray($model['id'] ?? $model),
    ...
    public function health(): bool
    {
        try {
            $response = $this->makeRequest('GET', '/v1/models', []);
            return isset($response['data']) || isset($response['models']);
        } catch (\Throwable) {
            return false;
        }
    }
